...
class Popup {
  ...
  init() {
    (this.id = this.params.id ?? "popup"),
      ...
      (this.css += `\n        .popup.${
        ...
        this.fixedHeight ? this.height : "fit-content"
      };\n            border-radius: ${
        this.borderRadius
    ...
    t.append(i),
      i.appendChild(document.createTextNode(this.css)),
    ...
    if (
      ...
      this.popupEl.addEventListener("click", (t) => {
        if (
          "popup-close" == t.target.className ||
          t.target.classList.contains("popup")
        ) {
      ...
      this.params.loadCallback &&
        "function" == typeof this.params.loadCallback &&
        this.params.loadCallback(),
      ...
      if (
        this.showOnce &&
        localStorage &&
        localStorage.getItem("popup-" + this.id)
      )
        return;
  ...
  hide() {
    this.popupEl.classList.remove("fade-in"),
      ...
      localStorage &&
        this.showOnce &&
        localStorage.setItem("popup-" + this.id, !0),
      postHide(this);
  }
}
function postShow(t) {
...
function postHide(t) {
  t.params.hideCallback &&
    "function" == typeof t.params.hideCallback &&
    t.params.hideCallback(),
    enableScroll();
}
